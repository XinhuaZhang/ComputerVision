...
module Application.KDTree.KdMapStatic
       ( -- * Usage
...
inRadiusCount
  :: (Real a)
  => KdMap a p v -> a -> p -> Int
inRadiusCount (KdMap pointAsList distSqr t _) radius query =
  go (cycle $ pointAsList query) t
  where
    go [] _ = error "inRadius.go: no empty lists allowed!"
    go _ Empty = 0
    go (queryAxisValue:qvs) (TreeNode left (k, v) nodeAxisVal right) =
      let onTheLeft = queryAxisValue <= nodeAxisVal
          ...
          accAfterOffside =
            if abs (queryAxisValue - nodeAxisVal) < radius
              then if onTheLeft
                     then (go qvs right)
                     else (go qvs left)
              else 0
          accAfterCurrent =
            if distSqr k query <= radius * radius
              then accAfterOnside + accAfterOffside + 1
              else accAfterOnside + accAfterOffside
      in force accAfterCurrent
