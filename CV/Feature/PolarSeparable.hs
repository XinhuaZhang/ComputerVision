...
magnitudeConduitDouble
  ...
  -> Int -> Acc (A.Array DIM3 Double) -> Acc (A.Array DIM3 Double)
  -> Conduit (AU.Array (Int, Int, Int) Double) IO [PolarSeparableFeaturePoint]
magnitudeConduitDouble parallelParams ctx filter factor meanArr varArr = do
  xs <- CL.take (batchSize parallelParams)
  if P.length xs > 0
    then do
      let (_, (nx', ny', nfOld)) = bounds . P.head $ xs
            ...
            if factor == 1
              then P.map
                     ...
                     (P.map toIArray .
                      multiGPUStream
                        ctx
                        (applyFilter filter >-> A.map A.real >->
                         crop25D
                           (div (nx - nxNew) 2)
                           (div (ny - nyNew) 2)
                           nxNew
